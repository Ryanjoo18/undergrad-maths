...
\item \vocab{equivalence}, denoted by $P\equiv Q$, means $P$ and $Q$ are logically equivalent statements;

\item \vocab{conjunction}, denoted by $P\land Q$, means ``$P$ and $Q$'';

\item \vocab{disjunction}, denoted by $P\lor Q$, means ``$P$ or $Q$'';

\item \vocab{negation}, denoted by $\lnot P$, means ``not $P$''.
\end{enumerate}
...
We denote an \vocab{implication} by 
\[P\implies Q\]
...
We denote a \vocab{bidirectional implication} by
\[P\iff Q\]
...
\begin{itemize}
\item 
A \vocab{direct proof} of $P \implies Q$ is a series of valid arguments that start with the hypothesis $P$ and end with the conclusion $Q$.
\[P\implies\cdots\implies Q\]

\item A \vocab{proof by contrapositive} of $P \implies Q$ is to prove instead $\lnot Q \implies \lnot P$.

\item A \vocab{disproof by counterexample} is to provide a counterexample to disprove a statement, which makes the negation of the statement true.
...
\item A \vocab{proof by cases} is to first dividing the situation into cases which exhaust all the possibilities, and then show that the statement follows in all cases.
\end{itemize}
\pagebreak

\subsection{Proof by Contradiction}
To \vocab{prove by contradiction},
\begin{enumerate}
...
To \vocab{prove uniqueness} (after proving existence), we can either
\begin{itemize}
