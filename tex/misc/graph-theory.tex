...
A \vocab{graph}\index{graph} $\mathcal{G}$ is a pair $\brac{V(\mathcal{G}),E(\mathcal{G})}$. $V(\mathcal{G})$ is the \emph{vertex set}, whose elements are called \emph{vertices} of $\mathcal{G}$; $E(\mathcal{G})$ is the \emph{edge set}, whose elements are called \emph{edges} of $\mathcal{G}$.
\end{definition}
...
\begin{definition}
The \vocab{order} of a graph $\mathcal{G}$ is $|V(\mathcal{G})|$ (number of vertices). The \vocab{size} of $\mathcal{G}$ is $|E(\mathcal{G})|$ (number of edges, also denoted by $e(\mathcal{G})$).
\end{definition}
...
\begin{definition}
A \vocab{loop} is an edge $vv$ for some $v\in V(\mathcal{G})$; that is, an edge whose endpoints are equal. An edge is a \vocab{multiple edge} if it appears more than once in $E(\mathcal{G})$.
...
\begin{definition}
$u,v\in V(\mathcal{G})$ are called \vocab{neighbours} if $uv\in E(\mathcal{G})$; we also say that $u$ and $v$ are \emph{adjacent}, denoted by $u\leftrightarrow v$.
\end{definition}

\begin{definition}
The \vocab{complement} $\mathcal{G}$ of a simple graph $\overline{\mathcal{G}}$ is the simple graph with $V(\mathcal{G})$ defined by
...
$\mathcal{G}$ is called \vocab{bipartite} if $V(\mathcal{G})$ is the union of two disjoint independent sets, called \emph{partite sets} of $\mathcal{G}$.

%$\mathcal{G}$ is said to be \vocab{complete bipartite} if $\mathcal{G}$ is bipartite and all possible edges between the two sets $A$ and $B$ are drawn. In the case where $|A|=m, |B|=n$, such a graph is denoted by $K_{m,n}$.
\end{definition}

\begin{definition}
The \vocab{chromatic number} of a graph $\mathcal{G}$, denoted by $\chi(\mathcal{G})$, is the minimum number of colours needed to label the vertices so that adjacent vertices receive different colours.

Let $k\ge2$. $\mathcal{G}$ is said to be \vocab{$k$-partite} if $V(\mathcal{G})$ expressed as the union of $k$ independent sets.
...
A \vocab{$u-v$ walk} in $\mathcal{G}$, denoted by $W$, is a finite sequence of vertices 
...
\item A \vocab{cycle} is a closed walk without repeating vertices, other than the initial and terminal vertices, i.e. $u=v$ but the vertices are otherwise distinct and $W$ has at least 3 vertices. If a graph $\mathcal{G}$ has no cycle we call it \vocab{acyclic}.
...
\begin{definition}
Given $v\in V(\mathcal{G})$, the \vocab{degree} of $v$, denoted by $d(v)$, is the number of neighbours of $v$ in $\mathcal{G}$. If the degree of each vertex is the same, we can call that the \emph{degree} of the graph.

$v\in V(\mathcal{G})$ is called a \vocab{leaf} if $d(v)=1$; that is, it has one unique neighbour.
\end{definition}
...
$\mathcal{H}$ is a \vocab{subgraph} of $\mathcal{G}$ if $V(\mathcal{H})\subset V(\mathcal{G})$ and $E(\mathcal{H})\subset E(\mathcal{G})$. Furthermore,
...
A graph $\mathcal{G}$ is \vocab{connected} if there exists a $u-v$ path for all $u,v\in V(\mathcal{G})$.

Vertices $u,v\in V(\mathcal{G})$ \vocab{lie in the same component} if they are joined by a $u-v$ walk. Clearly this forms an equivalence relation and the partition of $V(\mathcal{G})$ into equivalence classes expresses $\mathcal{G}$ as a union of disjoint connected graphs called its \vocab{components}.
\end{definition}

\begin{definition}[Diameter]
The \vocab{diameter} of a connected graph $\mathcal{G}$, denoted by $\diam G$, is defined as
\[\diam G=\max_{u\neq v}d(u,v).\]
By construction, $d(u,v) \le \diam G$ for all $u,v \in V(\mathcal{G})$.
\end{definition}

\begin{definition}[Complete graph]
$\mathcal{G}$ is \vocab{complete} if every pair of vertices in $\mathcal{G}$ is joined by an edge. A complete graph on $n$ vertices is denoted by $K_n$.
\end{definition}

\begin{definition}
A graph $\mathcal{G}$ is \vocab{planar} if it can be drawn such that a pair of edges can only cross at a vertex.
\end{definition}
...
We conclude this section with another characterisation of trees. First we note that any connected graph $\mathcal{G}$ contains a minimally connected subgraph (i.e. a tree) with the same vertex set, which we call a \vocab{spanning tree} of $\mathcal{G}$.
...
We now mathematically formulate this problem. Let $\mathcal{G}$ be a graph. We say $M\subseteq E(\mathcal{G})$ is a \emph{matching} if the edges in $M$ are pairwise disjoint. We say $M$ is \emph{perfect} if every vertex belongs to some edge of $M$. 
...
\begin{definition}
Let $W$ be a walk in a graph $\mathcal{G}$. We call $W$ an \vocab{Euler trail} if every edge of $\mathcal{G}$ appears exactly once in $W$. 
...
A \vocab{Hamiltonian cycle} is a closed walk that visits every vertex of a graph $\mathcal{G}$ once.

When a graph $\mathcal{G}$ contains such a cycle, it is said to be \vocab{Hamiltonian}.
\end{definition}
