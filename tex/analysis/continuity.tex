...
\begin{lemma}[Arithmetic properties]\label{lemma:limit-function-arithmetic}
...
Suppose $E\subset X$. We say $f\colon E\to Y$ is \vocab{continuous}\index{continuity} at $p\in E$ if 
...
\begin{lemma}\label{lemma:continuity-preimage-open}
...
The \vocab{oscillation} $o(f,p)$ of $f$ at $p$ is defined by
...
The next result states that if the graph of $f\colon [a,b]\to\RR$ lies above the $x$-axis at $a$ and below the $x$-axis at $b$, then the graph must cross the axis somewhere in between. (This should be intuitively obvious.)
...
We say $f\colon X\to Y$ is \vocab{uniformly continuous}\index{continuity!uniform continuity} on $X$ if
...
Let $f\colon (a,b)\to\RR$. Let $x\in[a,b)$. The \vocab{right-hand limit}, denoted by $f(x+)$ or $\displaystyle\lim_{t\to x^+}f(t)$, exists if
...
We say $f\colon X\to Y$ is \vocab{Lipschitz continuous} if there exists $K\ge0$ such that
...
We say $f\colon X\to Y$ is a \emph{contraction} if it is a $K$-Lipschitz map for some $K<1$.
