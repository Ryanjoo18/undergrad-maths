...
A \vocab{partition} $P$ of a closed interval $[a,b]$ is a finite set of points, say
\[P=\{x_0,x_1,\dots,x_n\},\]
...
$f$ is \vocab{Riemann--Stieltjes integrable}\index{Riemann--Stieltjes integrability} with respect to $\alpha$ over $[a,b]$, if
...
Note the following special case: Take $\alpha(x)=x$. Then $\beta=\phi$. Assume $\phi^\prime\in\mathcal{R}[A,B]$. If Theorem 6.17 is applied to the left side of (37), we obtain
...
A \vocab{curve} in $\RR^k$ is a continuous mapping $\gamma:[a,b]\to\RR^k$.
...
$\gamma$ is \vocab{rectifiable} if $\Lambda(\gamma)<\infty$.
\end{definition}
